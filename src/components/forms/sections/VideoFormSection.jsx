...
import VIDEO_FORM_LABELS from '@/components/forms/labels/videoFormLabels'; // create this file or reuse image labels
...
/* Modular sub‑components (same as for images) */
...
/* Helper: build options for dropdowns */
...
const VideoFormSection = ({
  ...
  onFieldChange,
  colors = {},
  relatedData,   // contains { artworks, events, projects, etc. }
  dataState,
}) => {
  ...
  /* ── Tab schema for video ── */
  const VIDEO_SCHEMA = [
    { key: 'basic', fields: [{ name: 'video_url', type: 'text' }] },
  ...
  const enhancedSchema = VIDEO_SCHEMA.map((section) => ({
    ...section,
    ...(VIDEO_FORM_LABELS?.tabs?.[section.key] && { label: VIDEO_FORM_LABELS.tabs[section.key] }),
    ...(section.fields && {
      fields: section.fields.map((f) => ({
        ...f,
        label: VIDEO_FORM_LABELS?.fields?.[f.name] || { en: f.name, cn: f.name },
      })),
    }),
  }));

  /* Custom renderer for the tags section (reusing the same sub‑components) */
  ...
  return (
    <TabbedFormManager
      ...
      getLabelFunc={(key) => {
        const obj = VIDEO_FORM_LABELS?.tabs?.[key] || VIDEO_FORM_LABELS?.fields?.[key];

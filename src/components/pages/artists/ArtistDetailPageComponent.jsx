...
//        ▸ News font family (labelFont, useFont() no-role), but sized as
//          quiet section dividers: 15px, weight 300 (light), 0.01em,
//          not uppercase. Kept in sync with the exhibition/fair detail
//          page's SECTION_HEADING.
...
const CONFIG = {
  ...
  //   HEADING is a quiet section divider (labelFont for the News font
  //   family, but 15px / weight 300 — not the full News heading size).
  //   ↩ old small divider: 14px / weight 600 / 0.05em / uppercase.
  // ==========================================================================
  RELATED: {
    HEADING_FONT_SIZE_DESKTOP: "15px",
    HEADING_FONT_SIZE_MOBILE: "15px",
    HEADING_FONT_WEIGHT: 300,          // thin/light — not bold
...
const CollectionSection = memo(function CollectionSection({
  ...
  isMobile,
  textColor,
  labelFont, // ▸ News font family for the section label
  bioFont,
  metaFont,
}) {
  const C = CONFIG.COLLECTIONS;
...
// ============================================================================
// News (bibliography) section — same visual pattern as Collections
// ============================================================================
const getSafeNewsHref = (item) => item?.web_url || item?.pdf_url || item?.video_url || "#";
...
const NewsItem = memo(function NewsItem({
  ...
  const title = item?.title || (isCn ? "无题" : "Untitled");
  const meta = [
    item?.year || item?.date || item?.published_at,
    item?.author,
  ]
    .filter(Boolean)
    .join(", ");
  const href = getSafeNewsHref(item);
...
export default function ArtistDetailPageComponent({ artistSlug }) {
  ...
  // Featured right-column slideshow used to be sourced ONLY from the
  // artist's OWN ARTWORKS, with no exhibition-image concept at all.
  // Uncomment this line + swap the JSX block below back to using
  // `withImages` directly to fully restore that old behavior.
  // ----------------------------------------------------------------------
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {/*
                ================================================================
                ORIGINAL (commented out per request — DO NOT DELETE):
                Right-column featured slideshow used ONLY the artist's OWN
                ARTWORKS (`withImages`), with no exhibition-image fallback
                logic at all. Uncomment this block (and change the JSX below
                back to `withImages.length > 0 ? ... withImages ...`) to
                fully restore this old, artwork-only behavior.
                ================================================================

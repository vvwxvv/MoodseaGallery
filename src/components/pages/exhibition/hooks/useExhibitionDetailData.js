...
export default function useExhibitionDetailData(slug, isCn) {
  ...
  //
  //   1) Exhibition → Artwork: exhibition.related_artwork_title contains
  //      the artwork's title.
  ...
  const matchedArtworks = useMemo(() => {
    ...
    const relatedTitleSet = new Set(
      toArray(exhibition.related_artwork_title).map(normalizeTitle).filter(Boolean)
    );
    ...
    for (const aw of artworks) {
      ...
      const matchesFromExhibitionSide =
        !!artworkTitleKey && relatedTitleSet.has(artworkTitleKey);

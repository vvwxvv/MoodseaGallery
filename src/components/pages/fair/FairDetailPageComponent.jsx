...
export default function FairDetailPageComponent() {
  ...
  // Related works come from the hook, which resolves the fair's artwork
  // references against the Artwork collection and orders them by each
  // artwork's own `order.art_fair_page_order` (see useFairDetailData).
                    ...
                    {relatedArtworks.map((work, idx) => {
                      ...
                      return (
                        <HoverUnderlineLink
                          key={`work-${idx}`}
                          label={label}
                          href={`/artworks?title=${encodeURIComponent(label)}`}
                          index={idx}
                          isMobile={isMobile}

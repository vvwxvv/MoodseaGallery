...
export const createImageControlPanelConfig = () => {
  return {
    ...
    controls: [
      {
        type: 'select',
        action: 'selectGroupBy',
        valueKey: 'activeGroupBy',
        pinned: true,
        label: { cn: '分组', en: 'Group by' },
        tooltip: { cn: '选择分组方式', en: 'Choose how images are grouped' },
        options: [
          { value: 'tag', label: { cn: '标签 / 标题', en: 'Tag / Title' } },
          { value: 'artist', label: { cn: '艺术家', en: 'Artist' } },
        ],
      },
      {
        type: 'toggle',

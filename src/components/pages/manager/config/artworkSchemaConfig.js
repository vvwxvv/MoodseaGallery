...
import { FIXED_SIZE_PRESET } from '@/components/pages/manager/constants/cardDisplayPresets';
...
export const artworkSchemaConfig = {
  ...
  uiConfig: {
    ...
    // Uniform media box: every card keeps the same 4:3 shape and the image is
    // cropped to fill (`cover`), so varying image sizes never leave big gaps.
    ...FIXED_SIZE_PRESET,
    aspectRatio: '4 / 3',
  },
...
export const createArtworkControlPanelConfig = () => ({
  ...
  controls: [
    {
      type: 'select',
      action: 'selectOrderKey',
      valueKey: 'activeOrderKey',
      pinned: true,
      label: { cn: '排序', en: 'Order by' },
      tooltip: { cn: '选择排序维度', en: 'Choose which order to arrange by' },
      options: [
        { value: 'artist_page_order', label: { cn: '艺术家页排序', en: 'Artist Page Order' } },
        { value: 'exhibition_page_order', label: { cn: '展览页排序', en: 'Exhibition Page Order' } },
        { value: 'art_fair_page_order', label: { cn: '艺博会页排序', en: 'Art Fair Page Order' } },
      ],
    },
    {
      type: 'toggle',
